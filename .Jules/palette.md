...
## 2026-08-17 - Conveying Exact Semantic Ratios on Progress Bars
**Learning:** While `aria-valuenow` conveys a progress bar's state, it is often tied to a percentage (e.g., `0` to `100`). For abstract constraints representing discrete items (like "10 out of 52 cards drawn"), relying solely on a percentage provides less context to screen reader users than to sighted users who can see adjacent text labels.
**Action:** Always complement `role="progressbar"`, `aria-valuenow`, `aria-valuemin`, and `aria-valuemax` with the `aria-valuetext` attribute when the progress represents a discrete, bounded pool of items. Use `aria-valuetext` to convey the exact semantic ratio (e.g., "10 of 52") rather than just a percentage, ensuring an equivalent experience.
