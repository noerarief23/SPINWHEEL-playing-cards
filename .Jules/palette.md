...
## 2026-08-18 - Semantic Context for Progress Bars
**Learning:** Found an accessibility issue where the progress bar representing the deck state only announced a raw percentage (like "10%") via `aria-valuenow` to screen readers. For abstract constraints like a deck of cards, a raw percentage loses its semantic meaning compared to the exact ratio (e.g. "5 of 52 cards drawn"). Relying only on `aria-valuenow` creates a disconnect between the visual state (which can be inferred) and the screen reader output.
**Action:** Always complement `aria-valuenow` with `aria-valuetext` when the progress bar represents a concrete ratio or abstract constraint where a raw percentage lacks context. Also add a `title` attribute for parity so sighted users can verify the exact ratio on hover.
