...
## 2026-08-20 - Redundant screen reader announcements for aria-live regions
**Learning:** Found an accessibility issue pattern where `aria-live` attributes were placed directly on interactive elements (like buttons) while a separate, dynamically injected `aria-live` region (e.g., via a `showFeedback` helper) was also used to announce state changes or feedback for that same element. This causes redundant double-reading by screen readers.
**Action:** Avoid placing `aria-live` attributes directly on interactive elements (like buttons) if a separate, dynamically injected `aria-live` region is used to announce state changes or feedback for that element, to prevent redundant double-reading.
